...
public class CaptchaServiceProcessResponseTest {
	...
	public void testSuccessfulReCaptchaVerification() {
		...
		when(restTemplate.getForObject(new URI(
				"https://www.google.com/recaptcha/api/siteverify?secret=secret&response=validResponse&remoteip=clientIP"),
	...
	public void testInvalidReCaptchaVerification() {
		...
		when(restTemplate.getForObject(new URI(
				"https://www.google.com/recaptcha/api/siteverify?secret=secret&response=invalidResponse&remoteip=clientIP"),
				GoogleResponse.class))
			.thenReturn(googleResponse);
	...
	public void testClientErrorHandling() {
		...
		when(restTemplate.getForObject(new URI(
				"https://www.google.com/recaptcha/api/siteverify?secret=secret&response=errorResponse&remoteip=clientIP"),
				GoogleResponse.class))
			.thenReturn(googleResponse);
		assertThrows(ReCaptchaInvalidException.class, () -> captchaService.processResponse("errorResponse"));
    ...
    public void testRestClientExceptionHandling() {
        when(restTemplate.getForObject(new URI("https://www.google.com/recaptcha/api/siteverify?secret=secret&response=anyResponse&remoteip=clientIP"), GoogleResponse.class)).thenThrow(RestClientException.class);
