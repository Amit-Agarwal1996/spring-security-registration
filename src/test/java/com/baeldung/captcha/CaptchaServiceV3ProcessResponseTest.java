...
public class CaptchaServiceV3ProcessResponseTest {
	...
	private final String uri = String.format(
			"https://www.google.com/recaptcha/api/siteverify?secret=%s&response=%s&remoteip=%s", secret, response,
			clientIP);
    ...
    public void processResponseSuccessfulValidation() {
        when(captchaServiceV3.getReCaptchaSecret()).thenReturn(secret);
        when(captchaServiceV3.getClientIP()).thenReturn(clientIP);
        when(restTemplate.getForObject(URI.create(uri), GoogleResponse.class)).thenReturn(googleResponse);
        when(googleResponse.isSuccess()).thenReturn(true);
        when(googleResponse.getAction()).thenReturn(action);
        when(googleResponse.getScore()).thenReturn(threshold);
        when(captchaSettings.getThreshold()).thenReturn(threshold);
        captchaServiceV3.processResponse(response, action);
        verify(reCaptchaAttemptService, times(1)).reCaptchaSucceeded(clientIP);
    }
    ...
    public void processResponseFailureDueToClientError() {
        ...
        assertThrows(ReCaptchaInvalidException.class, () -> captchaServiceV3.processResponse(response, action));
        verify(reCaptchaAttemptService, times(1)).reCaptchaFailed(clientIP);
    }
    ...
    public void processResponseRegistrationUnavailable() {
        when(captchaServiceV3.getReCaptchaSecret()).thenReturn(secret);
        when(captchaServiceV3.getClientIP()).thenReturn(clientIP);
        when(restTemplate.getForObject(URI.create(uri), GoogleResponse.class)).thenThrow(RestClientException.class);
        assertThrows(ReCaptchaUnavailableException.class, () -> captchaServiceV3.processResponse(response, action));
    }
